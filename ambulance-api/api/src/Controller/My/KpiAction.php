...
#[Route('/api/my/kpi', name: 'api_my_kpi', methods: ['GET'])]
class KpiAction extends AbstractController
{
    ...
    public function __invoke(Request $request, KernelInterface $kernel): JsonResponse
    {
        ...
        $kpiRepeat = $this->getKpiRepeat($startOfMonth, $endOfMonth, $userId, $payrollPayroll);

        $total = $kpiAverageBill['accrued'] + $kpiHospital['accrued'] + $kpiRepeat['accrued'];
    ...
    ): array
    {
        ...
        return [
            ...
            'baseKpi' => $kpiValue,
            'original' => $initialAmountByKPI,
            'kpi' => $rate,
            'accrued' => (int)($initialAmountByKPI * $rate),
        ];
    }

    private function getKpiAverageBill(
        DateTimeImmutable $startOfMonth,
        DateTimeImmutable $endOfMonth,
        int               $userId,
                          $payrollPayroll
